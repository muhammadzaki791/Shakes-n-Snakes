  ...
  async function handleArchiveSelected() {
    ...
    try {
      ...
      for (const [date, dateOrders] of Object.entries(dateGroups)) {
        ...
        completed.forEach((o) => {
          o.items.forEach((i) => {
            if (!itemCounts[i.menuItemTitle])
              itemCounts[i.menuItemTitle] = { quantity: 0, revenue: 0 };
        ...
        if (existing) {
          const prevData = existing.archivedOrdersData
            ? JSON.parse(existing.archivedOrdersData)
            : [];
          await adminWriteClient
            .patch(existing._id)
            .set({
              totalOrders: existing.totalOrders + dateOrders.length,
              completedOrders:
                (existing.completedOrders || 0) + completed.length,
              cancelledOrders:
                (existing.cancelledOrders || 0) + cancelled.length,
    ...
    <div className="bg-brand-bg min-h-screen py-12 px-4">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-6">
          <Link
            href="/admin"
            className="inline-flex items-center gap-1 text-sm text-brand-text-muted hover:text-brand-primary mb-3"
          >
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-brand-text-secondary text-sm mt-1">
                {statusCounts.all} total &middot; Today&apos;s revenue: Rs.{" "}
                {todayRevenue}
              </p>
            </div>
        ...
        >
          <TabsList className="bg-brand-elevated border border-white/10">
            <TabsTrigger
              value="all"
              className="data-[state=active]:bg-brand-primary data-[state=active]:text-white text-brand-text-secondary"
            >
              All ({statusCounts.all})
            </TabsTrigger>
            <TabsTrigger
              value="pending"
              className="data-[state=active]:bg-yellow-600 data-[state=active]:text-white text-brand-text-secondary"
            >
              Pending ({statusCounts.pending})
            </TabsTrigger>
            <TabsTrigger
              value="in-progress"
              className="data-[state=active]:bg-blue-600 data-[state=active]:text-white text-brand-text-secondary"
            >
              In Progress ({statusCounts["in-progress"]})
            </TabsTrigger>
            <TabsTrigger
              value="completed"
              className="data-[state=active]:bg-green-600 data-[state=active]:text-white text-brand-text-secondary"
            >
              Completed ({statusCounts.completed})
            </TabsTrigger>
            <TabsTrigger
              value="cancelled"
              className="data-[state=active]:bg-red-600 data-[state=active]:text-white text-brand-text-secondary"
            >
