...
export const teaCoffee = defineType({
  name: 'teaCoffee',
  title: 'Tea & Coffee',
  type: 'document',
  icon: () => '☕',
  fields: [
    ...
    defineField({
      name: 'drinkType', title: 'Drink Type', type: 'string',
      options: { list: ['Tea', 'Coffee', 'Milk Drink', 'Hot Chocolate', 'Specialty'] },
      validation: (r) => r.required(),
    }),
    defineField({
      name: 'milkOption', title: 'Milk Option', type: 'string',
      options: { list: ['Regular Milk', 'Low Fat', 'Oat Milk', 'No Milk', 'Pakola'] },
    }),
    defineField({ name: 'isHot', title: 'Served Hot', type: 'boolean', initialValue: true }),
    ...
    defineField({
      ...
      of: [{
        type: 'object',
        fields: [
          defineField({ name: 'sizeName', title: 'Size', type: 'string', options: { list: ['Small', 'Regular', 'Large'] } }),
  ...
  preview: {
    select: { title: 'title', subtitle: 'drinkType', media: 'images.0' },
  },
})
