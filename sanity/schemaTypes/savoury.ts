...
export const savoury = defineType({
  name: 'savoury',
  title: 'Savoury',
  type: 'document',
  icon: () => '🍔',
  fields: [
    ...
    defineField({
      ...
      options: { list: ['Fries', 'Pizza', 'Nuggets', 'Loaded Fries', 'Snack Platter'] },
    ...
    defineField({
      ...
      of: [{
        type: 'object',
        fields: [
          defineField({ name: 'sizeName', title: 'Size', type: 'string', options: { list: ['Regular', 'Large', 'Sharing', 'Family'] } }),
          defineField({ name: 'price', title: 'Price', type: 'string', validation: (r) => r.required() }),
        ],
      }],
      validation: (r) => r.required().min(1),
    }),
    defineField({ name: 'tags', title: 'Tags', type: 'array', of: [{ type: 'string' }], options: { list: ['Bestseller', 'New', 'Spicy', "Chef's Special", 'Seasonal'] } }),
